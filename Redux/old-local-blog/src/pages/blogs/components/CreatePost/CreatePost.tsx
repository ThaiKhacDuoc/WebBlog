...
import { createPost, cancelEditing, updatePost } from 'pages/blogs/blog.slice'
...
export default function CreatePost() {
  ...
  const [errorFrom, seterrorFrom] = useState<ErrorType | null>(null)
      ...
      <div className='mb-6'>
        <label
          ...
          className={`mb-2 block text-sm font-medium  dark:text-gray-300 ${
            errorFrom?.publishDate ? 'text-red-700' : 'text - gray - 900'
          }`}
        >
          Publish Date
        </label>
        <input
          ...
          className={`block w-56 rounded-lg border  p-2.5 text-sm  focus:outline-none ${
            errorFrom?.publishDate
              ? 'boder-red-500 bg-red-500 text-red-900 placeholder-red-700 focus-border-red-500 focus:ring-red-500'
        ...
        {errorFrom?.publishDate && (
          <p className='mt-2 text-sm text-red-600'>
            <span className='font-medium'>Lỗi! Không được đăng bài trong quá khứ</span>
          </p>
        )}
      </div>
